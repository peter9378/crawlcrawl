...
def crawl_naverprice(keyword: str, limit: int = 3):
    print("???")
    result = {
        'keyword' : keyword,
        'result'  : []
    ...
    try:
        scraper = Scraper()
        result = scraper.crawl_naverprice(keyword, limit)
        result = {
            'keyword': keyword,
            'result': result
        }
...
@app.get("/search/naverprice")
async def search_naverprice(keywords: str, limit: int = 3):
    logger = logging.getLogger('uvicorn')
    result = {
        'keyword' : keywords,
        'result'  : []
    ...
    try:
        ...
        result = await loop.run_in_executor(executor, crawl_naverprice, keywords, limit)
