...
app = FastAPI(
    title="Coupang Suggestion Scraper",
    description="쿠팡 연관검색어 크롤링 API",
    version="1.0.0"
)
...
def crawl_coupang_sync(keyword: str):
    logger.info(f"[WORKER] Starting crawl_coupang_sync for keyword: {keyword}")
    scraper = Scraper()
    try:
        result = scraper.get_coupang_suggestions(keyword)
        # scraper now returns the full dict structure
        return result
...
async def search_suggestions(keyword: str):
    ...
    try:
        ...
        result = await loop.run_in_executor(executor, crawl_coupang_sync, keyword)
        return result
    except Exception as e:
